...
class BankCardOCR:
    ...
    def __init__(self):
        # 请求地址
        self.url = "https://aip.baidubce.com/rest/2.0/ocr/v1/bankcard?access_token=" + self.get_access_token()
        self.payload = self.Payload()
    ...
    def get_result(self, image_path: str):
        ...
        response.encoding = "utf-8"
        return self.transformation_result(response.json())

    def transformation_result(self, result) -> dict | None:
        content = result.get("result", None)
        if content is not None:
            transformation_result = {
...
if __name__ == '__main__':
    # 可以测试新的类或保持原有功能
    ocr = BankCardOCR()
    print(ocr.get_result(r"E:\user_file\download\IMG_20260427_223834.jpg"))  # 需要替换为实际的图片路径
    # main()  # 如果需要运行原来的测试函数，取消注释
